...
export default async function generatePackageJson(name = '', dest = '', options = {}) {
  ...
  const devDependencies = [
          {name: '@babel/core', version: '^7.6.0'},
          {name: '@babel/preset-env', version: '^7.6.0'},
          {name: '@babel/register', version: '^7.6.0'},
          {name: '@hidoo/eslint-config', version: '^0.1.4'},
          {name: '@hidoo/util-fancy-print', version: gulpProjectVersion},
          {name: 'babel-eslint', version: '^10.0.3'},
          {name: 'commander', version: '^3.0.1'},
          {name: 'cross-env', version: '^5.2.1'},
          {name: 'eslint', version: '^6.3.0'},
          {name: 'gulp', version: '^4.0.2'},
          {name: 'husky', version: '^3.0.5'},
          {name: 'lint-staged', version: '^9.2.5'},
  ...
  if (options.css) {
    if (options.cssPreprocessor === 'sass') {
      devDependencies.push(
        ...
        {name: '@hidoo/stylelint-config', version: '^0.1.0'},
        {name: '@hidoo/kss-builder', version: '^0.3.1'},
        {name: 'stylelint', version: '^10.1.0'}
      );
    }
    else {
      devDependencies.push(
  ...
  if (options.conventionalCommits) {
    devDependencies.push(
      {name: '@commitlint/cli', version: '^8.1.0'},
      {name: '@commitlint/config-conventional', version: '^8.1.0'},
